...
STATIC uint32 *Base_Address[6] = { (volatile uint32*) GPIO_PORTA_BASE_ADDRESS,
                                   ...
                                   (volatile uint32*) GPIO_PORTE_BASE_ADDRESS,
                                   (volatile uint32*) GPIO_PORTF_BASE_ADDRESS };
...
void Port_Init(const Port_ConfigType *ConfigPtr)
{
...
#endif
    /*Changing the Port status to initialized*/
    ...
    for (index = 0; index < PORT_CONFIGURED_PINS; ++index)
    {
        ...
        if (((Port_ConfigPtr->Pin[index].port_num == 3)
                && (Port_ConfigPtr->Pin[index].pin_num == 7))
                || ((Port_ConfigPtr->Pin[index].port_num == 5)
                        && (Port_ConfigPtr->Pin[index].pin_num == 0))) /* PD7 or PF0 */
        {
        ...
        else if ((Port_ConfigPtr->Pin[index].port_num == 2)
        ...
        if (Port_ConfigPtr->Pin[index].initial_mode == PORT_PIN_MODE_DIO)
        {
            ...
            *(volatile uint32*) ((volatile uint8*) PortGpio_Ptr
                    ...
                    << (Port_ConfigPtr->Pin[index].pin_num * 4));
        ...
        else /* Another mode */
        {
            ...
            *(volatile uint32*) ((volatile uint8*) PortGpio_Ptr
                    + PORT_CTL_REG_OFFSET) |=
                    (Port_ConfigPtr->Pin[index].initial_mode
                            & 0x0000000F
                                    << (Port_ConfigPtr->Pin[index].pin_num * 4));
...
void Port_SetPinMode(Port_PinType Pin, Port_PinModeType Mode)
{
    ...
    else /* Another mode */
    {
        ...
        *(volatile uint32*) ((volatile uint8*) PortGpio_Ptr
                + PORT_CTL_REG_OFFSET) |=
                (Port_ConfigPtr->Pin[Pin].initial_mode
